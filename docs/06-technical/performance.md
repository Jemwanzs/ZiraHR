...
## Later addition: RouteProgressBar (perceived navigation speed)

Measured actual client-side navigation time (click to new route rendered, via a Puppeteer harness against the production build) before touching anything: warm/prefetched navigations between static pages resolve in **33–82ms** — already effectively instant, confirming `next/link`'s default viewport prefetching is doing its job and there was no real routing bottleneck to fix.

Added `RouteProgressBar` (`src/components/navigation/RouteProgressBar.tsx`) anyway, for the cases that *aren't* the warm/prefetched happy path: a throttled connection, or a click that lands before hydration finishes (in which case `next/link` hasn't attached its click handler yet and the browser falls back to a full native navigation). Without any feedback, that gap reads as "the site froze"; the thin top-of-page bar gives instant visual acknowledgement of every click regardless of how long the actual transition takes, closing a beat after the new route commits rather than snapping away instantly. Global click-listener based (catches every internal `<a>`, not just ones wrapped in a special component) rather than per-link, since it needs to catch navigation from *any* nav item, footer link, or CTA button site-wide.

One real bug surfaced while building this: the first implementation checked `event.defaultPrevented` to skip clicks it shouldn't react to, but `next/link` itself calls `preventDefault()` as part of *normal* client-side routing — so that check was silently skipping every real navigation it was built to catch. Fixed by dropping that check (the other guards — modifier keys, non-primary button, `target="_blank"`, `download`, same-path — are sufficient).

## Later addition: ProductScenes (shared "sample screen" system) + Hero depth pass

`HeroProductReel`'s four scenes (Payroll, Leave, Dashboard, Ask TiJa) were extracted into `src/components/sections/ProductScenes.tsx` so `ProductTourSection` ("see it in action, before you sign up") could reuse the exact same illustrated components instead of the generic icon-only placeholder it had before, plus two new scenes built the same way (`ApproveRequestScene`, `EmployeeProfileScene` — the latter reuses the "Amina" persona already established in the Employee Journey section rather than inventing a new fictional employee). All scene components now take a root-scoped translator (`useTranslations()` with no namespace, full dotted paths like `t("hero.reel.payroll.label")`) so the same component works from both call sites regardless of which translation namespace each page would otherwise default to.

Hero also got: content-relevant `PlaceholderIcon`s on the 7 satellite cards (previously all showed the same generic grid icon — a real, screenshot-caught bug), a plain-CSS entrance animation + hover lift on those cards (`hero-satellite-in` keyframe in `globals.css`, kept in sync with the sitewide reduced-motion policy via a `prefers-reduced-motion` override there), and a soft two-blob ambient background glow behind the hero text and inside the reel frame — decorative only, plain CSS/no Motion, so it can't reintroduce the Phase 9 LCP-gating regression (nothing about the hero text's render path changed). Re-verified after: Lighthouse unaffected, still meets all four 90+/95+ targets.

Also fixed while auditing: `DashboardsSection` had a locale-unsafe label bug — `"My Dashboard"` (already a complete phrase) had `" Dashboard"` appended again by a template, producing "My Dashboard Dashboard." Fixed by using the translated label as-is instead of a hardcoded English suffix (the earlier attempt to fix this with an English substring check would have broken French/Swahili, which don't contain the word "dashboard" at all in their translated form).

## Later addition: light/dark theme toggle + hero rework

Added a manual light/dark toggle (`ThemeProvider`, `src/components/theme/`), dark by default. Followed Next's own documented recipe for this (`node_modules/next/dist/docs/01-app/02-guides/preventing-flash-before-hydration.md`, "Themes" section) rather than a third-party dependency: `<html>` gets the `dark` class server-side by default, a tiny inline blocking `<script>` (`dangerouslySetInnerHTML`) in `<head>` corrects it to light *before first paint* if `localStorage` has a stored preference, and `suppressHydrationWarning` on `<html>` tells React to trust the DOM the script already corrected rather than flag a mismatch. `ThemeProvider` reads the current theme via `useSyncExternalStore` (reading `document.documentElement`'s class directly, with a `getServerSnapshot` matching the server default) rather than a `useEffect` that calls `setState` on mount — the first version did that and tripped the `react-hooks/set-state-in-effect` ESLint rule (calling setState synchronously in an effect causes an avoidable cascading render); `useSyncExternalStore` is the React-native tool for exactly this "read external mutable state, stay consistent through hydration" case.

The hero itself gained a pill announcement banner, an inline headline icon, and `AngledCardStack` (a decorative tilted-card cluster) — all plain CSS / server-rendered, same rule as the existing ambient glow blobs and satellite-card grid below, so none of it risks reintroducing the Phase 9 LCP regression documented above. Re-run Lighthouse against both themes after this change (dark is now the default first-paint state, which the existing audit numbers above predate).
